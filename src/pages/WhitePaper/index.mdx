...
where<br />
sToken_underlying = seller_deposit + accrued_premiums - default_payouts
...
Lastly, we use the buffer value(e.g. 0.05) to avoid the edge case inconsistency. For example, when the leverage ratio goes to the floor, without subtracting the buffer value, the premium curve steepens at a high pace and goes to 100%. The buffer values also ensure that numerator or denominator won’t be zero.
...
The MIN_CARAPACE_RISK_PREMIUM is the minimum amount of carapace_risk_premium paid by a protection buyer regardless of the leverage ratio.
...
*total_sTokens \* the_exchange_rate = total_sToken_underlying*<br />
