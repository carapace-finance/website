    ...
    <nav className="w-full flex h-[65px] md:h-[102px] px-[20px] md:px-[64px] justify-between items-center">
      <a href="/">
        <img
          src={assets.logo}
          alt="carapace"
          className="w-[145px] md:w-[245px] h-[43px] md:h-[62px] object-contain"
        />
      </a>
        ...
        {navLinks.map((nav, _) => (
          <li
            ...
            className={`decoration-buttonPink text-customDarkBlue list-none font-manrope font-normal cursor-pointer leading-[18px] tracking-[0.02em] text-[22px] ${
              active === nav.id
                ? "underline underline-offset-4 decoration-2 decoration-buttonPink decoration-solid"
                : ""
            } mr-10`}
            onClick={() => {
              setActive(nav.id);
          ...
          >
            <a
              className={`hover:text-customDarkBlue decoration-buttonPink ${
                active !== nav.id
                  ? "group transition-all duration-300 ease-in-out  hover:no-underline"
                  : "hover:decoration-buttonPink"
              }`}
              href={`${nav.id}`}
            >
              <span
                className={`bg-left-bottom bg-gradient-to-r from-buttonPink to-buttonPink bg-[length:0%_2px] bg-no-repeat group-hover:bg-[length:100%_2px] transition-all duration-500 ease-out"}`}
              >
                {nav.title}
              </span>
            </a>
            ...
            {navLinks.map((nav, _) => (
              ...
              >
                <a
                  className={`${
                    active === nav.id
                      ? "underline underline-offset-8 decoration-buttonPink"
                      : ""
